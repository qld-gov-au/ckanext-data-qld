...
def data_qld_user_name_validator(key, data, errors, context):
    ...
    if not is_sysadmin and 'publisher' in data[key].lower():
        errors[key].append('The username cannot contain the word \'publisher\'. Please enter another username.')
    ...
    if not is_sysadmin:
        ...
        for name in excluded_names:
            if name.lower() in data[key].lower():
                errors[key].append('The displayed name cannot contain certain words such as \'publisher\', \'QLD Government\' or similar. Please enter another display name.')
